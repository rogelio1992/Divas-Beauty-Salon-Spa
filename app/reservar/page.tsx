...
export default function Reserve() {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        fetch(`/api/public-booking?date=${date}&serviceId=${serviceId}&professional=${encodeURIComponent(professional)}`).then(r => r.json()).then(data => setSlots(data.slots ?? [])).finally(() => setLoading(false));
    ...
    async function submit(event: FormEvent<HTMLFormElement>) {
        ...
        setMessage(result.ok ? "¡Solicitud recibida! Divas Beauty Spa confirmará tu hora por WhatsApp." : result.error);
    ...
    return <main className="booking-page">
        <section className="booking-card">
            ...
            <p className="eyebrow">RESERVA ONLINE · SANTIAGO</p><h1>Reserva tu momento Divas</h1><p
            ...
            <form onSubmit={submit}><label>Nombre completo<input required name="clientName"
                ...
                <label>Hora disponible<select required name="time" disabled={!slots.length}>
                    ...
                    {slots.map(slot => <option key={slot}>{slot}</option>)}</select></label><input className="honeypot"
                                                                                                   ...
                                                                                                   tabIndex={-1}
                                                                                                   autoComplete="off"/>
                <button className="primary full">Solicitar reserva</button>
