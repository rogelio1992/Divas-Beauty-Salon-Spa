...
export default function Directory({view, services, clients, professionals, appointments, profile, onRefresh}: {
    ...
    return <>
        ...
        {detail && !editor && <Modal title={detail.full_name} onClose={() => setSelectedClient(null)}><p>{detail.phone || "Sin teléfono"} · {detail.email || "Sin correo"}</p>{message && <p className="notice" role="status">{message}</p>}<p className="client-notes">{detail.notes || "Sin notas registradas."}</p>{admin && <div className="directory-actions"><button className="view-appointment" disabled={busy} onClick={() => edit("client", detail.id)}>Editar ficha</button><button className="directory-delete" disabled={busy} onClick={() => void removeRecord("client", detail.id, detail.full_name)}>Eliminar clienta</button></div>}{admin && <ClientLoyalty key={detail.id} clientId={detail.id} phone={detail.phone}/>}<h3>Historial {admin ? "de citas" : "de tus citas"}</h3><div className="client-history">{appointments.filter(a => a.clientId === detail.id).sort((a,b) => `${b.date}${b.time}`.localeCompare(`${a.date}${a.time}`)).map(a => <article key={a.id}><strong>{a.date} · {a.time}</strong><p>{a.service} · {a.duration} min · {a.price == null ? "Valor no registrado" : money(a.price)}</p><span>{a.stylist} · {statuses[a.status] ?? a.status}</span></article>)}{!appointments.some(a => a.clientId === detail.id) && <p className="empty">Aún no hay citas.</p>}</div></Modal>}
