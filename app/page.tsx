...
const initialAppointments: Appointment[] = [
  { id: 1, time: "09:00", client: "Camila Rojas", service: "Esmaltado permanente", stylist: "Sofía", duration: 75, status: "Confirmada" },
  { id: 2, time: "10:30", client: "Valentina Díaz", service: "Pedicure spa", stylist: "Valentina", duration: 75, status: "Confirmada" },
  { id: 3, time: "13:00", client: "María Paz", service: "Lifting de pestañas", stylist: "Sofía", duration: 60, status: "Pendiente" },
  { id: 4, time: "15:30", client: "Antonia Silva", service: "Depilación facial", stylist: "Daniela", duration: 30, status: "Confirmada" }
];
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="content" id="agenda">
      <header><div><p className="eyebrow">SANTIAGO, CHILE · MARTES, 2 DE SEPTIEMBRE</p><h1>Buenos días, María <span>✦</span></h1><p className="subtle">Aquí tienes el resumen de Divas Beauty Spa.</p></div><div className="header-actions"><a className="whatsapp" href="https://wa.link/qf29bt" target="_blank" rel="noreferrer">◉ Reservas WhatsApp</a><button className="primary" onClick={() => setIsOpen(true)}>＋ Nueva cita</button></div></header>
    ...
    {isOpen && <div className="modal-backdrop"><form className="modal" onSubmit={addAppointment}><div className="modal-title"><div><p className="eyebrow">DIVAS BEAUTY SPA · AGENDA</p><h2>Nueva cita</h2></div><button type="button" onClick={() => setIsOpen(false)}>×</button></div><label>Cliente<input required name="client" placeholder="Nombre de la clienta" /></label><label>Servicio<select required name="service" defaultValue=""><option value="" disabled>Selecciona un servicio</option>{services.map((service) => <option key={service}>{service}</option>)}</select></label><div className="form-row"><label>Hora<input required name="time" type="time" defaultValue="10:00" /></label><label>Duración<select name="duration" defaultValue="60"><option value="30">30 min</option><option value="60">60 min</option><option value="75">75 min</option><option value="90">90 min</option><option value="120">120 min</option></select></label></div><label>Profesional<select name="stylist">{team.map((name) => <option key={name}>{name}</option>)}</select></label><button className="primary full" type="submit">Guardar cita</button></form></div>}
  </main>;
}
