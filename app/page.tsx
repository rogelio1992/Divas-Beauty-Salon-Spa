...
export default async function Salon() {
    ...
    return <div className={styles.site}>
        <header className={styles.header}><Link href="/" className={styles.brand} aria-label="Divas Beauty Spa, inicio">DIVAS<span>BEAUTY SPA</span></Link><nav className={styles.nav} aria-label="Navegación del salón"><a href="#servicios">Servicios</a>{gallery.length > 0 && <a href="#trabajos">Nuestros trabajos</a>}{promotions.length > 0 && <a href="#promociones">Promociones</a>}<a href="#contacto">Contacto</a></nav><Link className={styles.button} href="/reservar">Reservar hora ↗</Link></header>
        <main className={styles.main}>
            <section className={styles.hero}><div className={styles.heroCopy}><p className={styles.eyebrow}>BELLEZA · CUIDADO · TU MOMENTO</p><h1>{settings.headline}</h1><p className={styles.intro}>{settings.introduction}</p><div className={styles.actions}><Link className={styles.button} href="/reservar">Quiero reservar <span>↗</span></Link><a className={styles.textLink} href="#servicios">Explorar servicios ↓</a></div><p className={styles.heroNote}>Elige tu servicio y una hora disponible.<br/>Confirmaremos tu cita por WhatsApp.</p></div><div className={styles.art} aria-hidden="true"><div className={styles.artArch}><span className={styles.artStar}>✦</span><span className={styles.artWord}>divas</span><span className={styles.artCaption}>EL ARTE DE CUIDARTE</span></div><span className={styles.artStamp}>Un espacio<br/>para ti.</span></div></section>
        ...
        </main><footer className={styles.footer}><Link href="/" className={styles.brand}>DIVAS<span>BEAUTY SPA</span></Link><p>Un momento para ti.</p><Link href="/admin/login">Acceso del equipo ↗</Link></footer>
    </div>;
}
