...
export default function Home() {
  ...
  async function createAppointment(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from("appointments").insert({ client_name: form.get("client"), service_id: service.id, professional_name: form.get("professional"), starts_at: startsAt, duration_minutes: service.duration_minutes, status: "pending" });
    if (error) { setNotice("No se pudo guardar la cita. Inténtalo nuevamente."); return; }
    setOpen(false); setDate(String(form.get("date"))); setNotice("Cita guardada correctamente."); void loadData();
  }
...
function AppointmentForm({ date, services, onClose, onSubmit }: { date: string; services: Service[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { return <div className="modal-backdrop"><form className="modal" onSubmit={onSubmit}><div className="modal-title"><div><p className="eyebrow">DIVAS BEAUTY SPA · AGENDA</p><h2>Nueva cita</h2></div><button type="button" onClick={onClose}>×</button></div><label>Cliente<input required name="client" placeholder="Nombre de la clienta"/></label><label>Servicio<select required name="service" defaultValue=""><option disabled value="">Selecciona un servicio</option>{services.map(service => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label><div className="form-row"><label>Fecha<input required name="date" type="date" defaultValue={date}/></label><label>Hora<input required name="time" type="time" defaultValue="10:00"/></label></div><label>Profesional<select required name="professional" defaultValue=""><option disabled value="">Selecciona una profesional</option>{team.map(name => <option key={name}>{name}</option>)}</select></label><button className="primary full">Guardar cita</button></form></div>; }
